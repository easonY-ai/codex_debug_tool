# Epic/Story 拆分与进度

## 文档目的

本文档是项目的 SDD 交付台账，记录项目阶段、Epic/Story 范围、当前状态和人工验收门禁。产品语义仍以 PRD 为准，实现方式仍以技术设计为准。

## 状态定义

- 项目进度：需求澄清、原型设计、技术方案设计、代码开发、交付完成。
- Epic 进度：待启动、开发中（必须注明当前 Story）、开发完成。
- Story 进度：待启动、开发中、集成测试中（必须注明当前用例）。
- “代码已存在”不等于“Story 已完成”；单元测试、集成测试和用户人工验收都通过后，才能结束 Story。

## 项目总进度

| 项目 | 当前值 |
| --- | --- |
| 当前阶段 | 技术方案设计（已确认的 OTel-first 原型现按产品版本放在 `prototype-v1`；二维 Trace 时间轴迭代待人工评审，迁移 Story 待拆分） |
| 当前 Epic | E1 Execution 采集与 Trace 可视化 |
| 当前 Story | S2.2 OTel-first 数据源方案升级已完成；下一项迁移 Story 尚未拆分或启动 |
| 当前测试 | F-01、F-02、F-04、F-05 已通过；V1.0 原型二维时间轴的 8 项单元测试、TypeScript/构建及 1440px/390px Playwright 浏览器检查通过。新交互的人工评审未完成；F-03 hosted tool 与审批等待能力已移至 V1.1 |
| 下一门禁 | 用户评审二维 Trace 时间轴及目标实体、聚合边界、服务分层和 Trace 主链路；随后按顺序拆分 OTel-first 迁移 Story。每个 Story 启动前完成对应规格、具名接口 DTO、baseline 3 DDL/文件清单、测试用例及必要的技术方案评审，逐项开发和人工验收 |
| 后续优先 Story | S2.2 完成后拆分 OTel-first 迁移 Story；不恢复验收现有 Hook-first S2.1-S2，也不直接启动旧 S2.1-S3、S3 或 S3.1 |

## Epic 与 Story

### E0 规格、设计与领域架构基线

- 状态：开发完成。
- 产出：PRD、历史 Hook-first 原型（Git 历史）、当前产品 V1.0 原型、技术设计、数据关联模型、MySQL 切换方案、领域化架构升级规格。
- 注意：旧评审版通过 Git 历史追溯；当前目录只保留产品 V1.0 原型。

| Story | 范围 | 状态 | 当前证据/门禁 |
| --- | --- | --- | --- |
| S0 领域化架构升级 | 按问题子域拆分 DDD 分层；Hook 主链路结构化 DTO、Raw Event 特例、调度器/应用服务/领域模型/持久化边界 | 开发完成 | 用户已完成代码阅读；自动回归与 Story1 步骤 1–10 人工回归均通过 |

### E1 Execution 采集与 Trace 可视化

- 状态：开发中，S2.2 来源方案已确认；V1.0 原型的二维 Trace 时间轴待人工评审；下一项迁移 Story 待拆分，S2.1 暂停。
- 目标：以 OTel 建立执行与性能模型，用 Transcript 补充内容并在正式前端绘制 Trace；Hook 不进入目标架构，迁移完成前仅保留历史实现以支持回归和回滚。

| Story | 范围 | 状态 | 当前证据/门禁 |
| --- | --- | --- | --- |
| S1 真实 Codex Hook 主链路 | Python 3.11 运行环境、Hook 配置与信任、forwarder、Hook 接收、MySQL 骨架、列表与 Trace 行为节点 | 开发完成 | `IT-HOOK-TRACE-001` 步骤 1–10 已在 S0 后人工回归通过；用户于 2026-09-16 明确接受 Story |
| S1.1 本地运行日志 | CLI 与后端结构化按日日志、7 天保留、敏感字段脱敏；不含前端日志 | 开发完成 | CLI 6 项、后端 JDK 17/MySQL 全量 36 项及 `IT-LOG-001` 人工验收通过；用户于 2026-09-16 完成代码阅读并明确接受 Story |
| S2 transcript 内容补齐 | 路径安全、session_meta 校验、用户/工具/最终内容挂接与三源证据检查 | 开发完成 | 后端 JDK 17/MySQL 全量 49 项、前端 19 项及生产构建通过；`IT-HOOK-TRACE-002` 步骤 1–12 和代码阅读通过；用户于 2026-09-16 明确接受 Story |
| S2.1 问题域与聚合边界架构升级 | Execution、Transcript、Telemetry、Trace、Operations 边界和聚合升级 | 暂停（S2.1-S2 自动测试完成） | S2.1-S2 原实现及 61 项测试已通过但暂停人工验收；S2.2 将决定 Execution 的事实来源及当前 Hook-first 实现的复用、改造或废弃方式 |
| S2.2 OTel-first 数据源方案升级 | V1.0 采用 OTel + Transcript，OTel 作为核心行为遥测与性能来源，Transcript 作为内容来源，移除目标架构中的 Hook | 开发完成 | 来源方案、规格一致性及 V3 原型人工门禁均已确认；自动构建、6 项单元测试及浏览器检查通过。代码迁移留给后续逐项拆分的 Story，具体细节在对应 Story 调整 |
| S3 OTel 性能 Trace | OTLP traces 接收、工具关联、耗时摘要守恒、精度标识 | 待启动 | S2.2 规格与 V3 门禁完成，且新的 OTel-first 迁移 Story 通过验收后再决定启动；`EXACT` 仅适用于版本化样本已验证的本地工具模型 Call ID |
| S4 实时 Trace | SSE 更新、运行中到完成、缺失内容标记 | 待启动 | S3 验收后开始 |
| S5 健壮性与降级 | 幂等、乱序、半行、错配、UNKNOWN、缺源、同名并行歧义 | 待启动 | S4 验收后开始 |

### V1.1 TODO

- Hosted tool 完整生命周期：重新验证目标 Codex 版本是否提供单次 hosted tool 的稳定身份、开始/结束、状态、耗时和可见结果契约；验证通过后再设计持久化、关联和页面能力，不从 Turn 级结果反推。
- 审批等待完整分析：验证目标 Codex 版本是否提供稳定审批调用身份、等待开始、决策结束、允许/拒绝/超时状态及持续时间；验证通过后再设计 Span、工具关联、耗时守恒与“审批等待过长”诊断。不得从决策事件或相邻工具时间反推等待区间。

| 强关联后续 Story | 所属范围 | 状态 | 说明与启动条件 |
| --- | --- | --- | --- |
| S3.1 Java 服务层 OTel 关联重构 | E1-S3 OTel 性能 Trace | 待启动（待重新定义） | 按 OTel-to-Transcript 关系重新定义，不再以 Hook Tool 为目标；候选筛选、证据等级、算法版本和时间差计算仍必须位于 Java 服务层，Mapper 只负责持久化 |

### E2 完整性、指标与诊断

- 状态：待启动。
- 范围：V1.0 内完整的 API/TTFT、本地处理归因、异常耗时贡献、确定性诊断与全量统计口径；审批等待完整分析已移至 V1.1。
- 启动条件：E1 完成。当前实现中的 `modelRequestMs` 和 `localMs` 仍不代表 PRD 要求的完整能力；`approvalMs` 在 V1.0 必须为未知且不参与耗时分类或诊断，不能用旧占位值冒充已验证审批等待。

| 长期 Story | 范围 | 状态 | 说明 |
| --- | --- | --- | --- |
| E2-S1 运行指标持久化与高可用聚合 | 替换 Hook 接收服务中进程内计数器与样本列表；设计跨重启保留、多实例聚合、窗口/分位数计算、容量上限、降级与一致性语义 | 待启动（长期） | 当前 `requests`、`accepted`、`duplicates`、错误计数、`lastSuccessAt` 与 `processingSamples` 仅适用于单进程运行期，状态接口不得被解释为高可用或长期统计。开始前必须完成独立技术方案、数据模型和故障/并发测试 |

### E3 交付、安装与运维

- 状态：待启动。
- 范围：可复现打包、Hook 安装/检测、日志轮转、敏感信息扫描、完整交付验收。

## 进度更新规则

1. 每次开始 Story 前，先更新本文档和对应 Story 规格。
2. 开发中同步写单元测试；单元测试通过后，Story 才进入集成测试中。
3. 集成测试记录必须分开“自动执行”和“用户人工验收”，不得互相替代。
4. 未经用户明确同意，不得越过当前 Story 的人工门禁。
5. 暂缓 Story 仅登记优先级与边界；未补齐独立规格、测试用例并获用户启动指令前，不得实施。
6. 架构升级 Story 完成后，必须先回归受影响的已验收 Story，再解除后续集成测试门禁。
