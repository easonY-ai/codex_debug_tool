...
<template>
  <div class="app-shell">
    <header class="topbar">
      ...
      <div class="top-actions"><span class="prototype-badge">MOCK PROTOTYPE · V2</span><button class="icon-button" aria-label="设置"><el-icon><Setting /></el-icon></button><span class="avatar">TL</span></div>
    </header>
    <OverviewPage v-if="page === 'overview'" @open="openSession" />
    <TracePage v-else-if="page === 'trace'" :key="activeSession?.id" :session="activeSession ?? undefined" @back="go('overview')" />
