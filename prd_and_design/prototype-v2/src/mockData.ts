...
export const sessions: SessionSummary[] = [
  ...
  { id: 'turn-demo-005', sessionId: 'session-demo-e55b', sessionTitle: '数据库结构调整', turnId: 'turn-demo-005', title: '根据实体变更生成数据库迁移脚本', project: 'demo-data', model: 'gpt-5.6-sol', source: 'IDE', status: '成功', startedAt: '今天 10:04', durationMs: 39600, ttftMs: 4900, modelMs: 12600, toolMs: 22500, tokenUsage: 11920, completeness: '缺少性能', bottleneck: '等待工具审批', diagnosis: 'Hook 骨架与内容完整，约 18 秒无性能细分', confidence: '中' },
  { id: 'turn-demo-006', sessionId: 'session-demo-f21d', sessionTitle: '接口性能检查', turnId: 'turn-demo-006', title: '分析接口性能采样并指出慢请求', project: 'demo-api', model: 'gpt-5.6-sol', source: 'Desktop', status: '成功', startedAt: '昨天 18:42', durationMs: 32800, ttftMs: 7200, modelMs: 20900, toolMs: 7900, tokenUsage: 8320, completeness: '缺少内容', bottleneck: '模型请求链路', diagnosis: 'Hook 骨架与 OTel 已关联，缺少 JSONL 正文', confidence: '高' },
...
export const timelineItems: TimelineItem[] = [
  { id: 'evt-user-1', layer: 'Hook 行为', lane: '用户', title: '用户问题', startMs: 0, durationMs: 900, status: 'info', source: 'Hook + JSONL', input: '请定位支付回调测试偶发超时的原因，并运行相关测试。', description: 'Hook 建立轮次入口，JSONL 补齐用户正文。', contentStatus: '已补齐', timingPrecision: 'Hook 边界估算', hookRaw: { event: 'UserPromptSubmit', session_id: 'session-demo-a12f', turn_id: 'turn-demo-001', transcript_path: '/workspace/demo-project/sessions/demo-001.jsonl' }, raw: { timestamp: '2026-09-10T06:32:00.000Z', type: 'event_msg', payload: { type: 'user_message', message: '请定位支付回调测试偶发超时的原因，并运行相关测试。' } } },
  { id: 'evt-model-1', layer: 'Hook 行为', lane: '模型', title: '分析代码与制定步骤', startMs: 900, durationMs: 10800, status: 'success', source: 'Hook + JSONL', input: '用户问题 + 当前会话上下文（18 条消息）', output: '先定位支付回调处理器和相关超时配置，再运行最小范围测试复现问题。', description: 'Hook 提供轮次边界，JSONL 补齐可见摘要。', contentStatus: '已补齐', timingPrecision: 'Hook 边界估算', hookRaw: { event: 'UserPromptSubmit', session_id: 'session-demo-a12f', turn_id: 'turn-demo-001' }, raw: { timestamp: '2026-09-10T06:32:00.900Z', type: 'response_item', payload: { type: 'message', role: 'assistant', content: [{ type: 'output_text', text: '定位回调处理器和超时配置，然后运行最小范围测试。' }] } } },
  { id: 'evt-tool-1', layer: 'Hook 行为', lane: '工具', title: 'rg 搜索回调处理器', startMs: 11800, durationMs: 3300, status: 'success', source: 'Hook + JSONL', callId: 'call_demo_rg_1', toolUseId: 'tooluse_demo_rg_1', input: 'rg "callback|timeout" /workspace/demo-project', output: 'src/payment/callback.ts:48: const RETRY_TIMEOUT = 10000\ntests/payment-callback.spec.ts:72: retryDelay: 10000', description: 'Hook 建立工具调用，JSONL 补齐参数与结果。', contentStatus: '已补齐', timingPrecision: 'Hook 边界估算', hookRaw: { event: 'PostToolUse', session_id: 'session-demo-a12f', turn_id: 'turn-demo-001', tool_use_id: 'tooluse_demo_rg_1', tool_name: 'shell' }, raw: { timestamp: '2026-09-10T06:32:11.800Z', type: 'response_item', payload: { type: 'custom_tool_call', call_id: 'call_demo_rg_1', name: 'shell' } } },
  { id: 'evt-tool-2', layer: 'Hook 行为', lane: '工具', title: '运行支付回调测试', startMs: 16900, durationMs: 42100, status: 'warning', source: 'Hook + JSONL', callId: 'call_demo_test_7', toolUseId: 'tooluse_demo_test_7', input: 'npm test -- payment-callback', output: 'PASS tests/payment-callback.spec.ts (41.2 s)\n3 tests passed；其中 retry fallback 等待约 30 秒。', description: 'Hook 建立工具调用，JSONL 补齐参数与结果，OTel 提供精确耗时。', contentStatus: '已补齐', timingPrecision: 'OTel 精确', hookRaw: { event: 'PostToolUse', session_id: 'session-demo-a12f', turn_id: 'turn-demo-001', tool_use_id: 'tooluse_demo_test_7', tool_name: 'shell' }, raw: { timestamp: '2026-09-10T06:32:16.900Z', type: 'response_item', payload: { type: 'custom_tool_call', call_id: 'call_demo_test_7', name: 'shell' } } },
  { id: 'evt-tool-exact', layer: 'Hook 行为', lane: '工具', title: '读取测试摘要', startMs: 59000, durationMs: 300, status: 'success', source: 'Hook + JSONL', callId: 'call_demo_exact_9', toolUseId: 'call_demo_exact_9', input: '读取合成测试摘要', output: '3 tests passed', description: '已验证格式中两侧标识相同的精确补齐样本。', contentStatus: '已补齐', timingPrecision: 'OTel 精确', hookRaw: { event: 'PostToolUse', session_id: 'session-demo-a12f', turn_id: 'turn-demo-001', tool_use_id: 'call_demo_exact_9', tool_name: 'read_file' }, raw: { timestamp: '2026-09-10T06:32:59.000Z', type: 'response_item', payload: { type: 'custom_tool_call', call_id: 'call_demo_exact_9', name: 'read_file' } } },
  { id: 'evt-model-2', layer: 'Hook 行为', lane: '模型', title: '归纳测试输出', startMs: 59400, durationMs: 11900, status: 'success', source: 'Hook + JSONL', input: '测试输出：3 tests passed；retry fallback 等待约 30 秒。', output: '主要耗时集中在测试桩的重试退避，并非支付回调业务逻辑本身。', description: 'JSONL 为 Hook 轮次补齐可见模型输出。', contentStatus: '已补齐', timingPrecision: 'Hook 边界估算', hookRaw: { event: 'Stop', session_id: 'session-demo-a12f', turn_id: 'turn-demo-001' }, raw: { timestamp: '2026-09-10T06:32:59.400Z', type: 'response_item', payload: { type: 'message', role: 'assistant', content: [{ type: 'output_text', text: '主要耗时集中在测试桩的重试退避。' }] } } },
  { id: 'evt-final-1', layer: 'Hook 行为', lane: '模型', title: '最终输出', startMs: 71900, durationMs: 6100, status: 'success', source: 'Hook + JSONL', input: '代码搜索结果、测试输出和当前轮次上下文', output: '定位完成：超时来自测试桩 retry fallback 的固定等待。建议在测试环境注入零延迟退避策略，并保留一条覆盖真实退避的集成测试。', description: 'Hook 标记轮次结束，JSONL 补齐最终可见回复。', contentStatus: '已补齐', timingPrecision: 'Hook 边界估算', hookRaw: { event: 'Stop', session_id: 'session-demo-a12f', turn_id: 'turn-demo-001' }, raw: { timestamp: '2026-09-10T06:33:11.900Z', type: 'response_item', payload: { type: 'message', role: 'assistant', phase: 'final_answer', content: [{ type: 'output_text', text: '定位完成：超时来自测试桩 retry fallback 的固定等待。' }] } } },
  ...
  { id: 'span-tool-1', layer: 'OTel 性能', lane: '工具 Span', title: 'shell · rg', startMs: 11950, durationMs: 3080, status: 'success', source: 'OTel Trace', traceId: 'trace_demo_a91', callId: 'call_demo_rg_1', description: '搜索工具执行 Span；当前适配器已验证它与 Hook 工具节点的映射。', raw: { trace_id: 'trace_demo_a91', span_id: 'span_tool_02', 'codex.call_id': 'call_demo_rg_1' } },
  ...
  { id: 'span-tool-exact', layer: 'OTel 性能', lane: '工具 Span', title: 'read_file · test summary', startMs: 59020, durationMs: 260, status: 'success', source: 'OTel Trace', traceId: 'trace_demo_a91', callId: 'call_demo_exact_9', description: '与已验证 Hook 工具节点精确对应。', raw: { trace_id: 'trace_demo_a91', span_id: 'span_tool_exact_09', 'codex.call_id': 'call_demo_exact_9' } },
...
export const alignments: Alignment[] = [
  { id: 'align-1', agentEventId: 'evt-tool-1', performanceSpanId: 'span-tool-1', level: 'INFERRED', evidence: '推测关联：标识不同；同一 Turn 内工具类型相同，时间重叠 92%。', timeDeltaMs: 150 },
  { id: 'align-2', agentEventId: 'evt-tool-2', performanceSpanId: 'span-tool-2', level: 'INFERRED', evidence: '推测关联：标识不同；同一 Turn 内工具类型相同，时间重叠 92%。', timeDeltaMs: 200 },
  { id: 'align-exact', agentEventId: 'evt-tool-exact', performanceSpanId: 'span-tool-exact', level: 'EXACT', evidence: '精确关联：Hook tool_use_id 与 JSONL call_id 值相同，格式适配器版本 codex-2026-09 已验证。', timeDeltaMs: 20 },
  ...
  { id: 'align-4', agentEventId: 'evt-model-2', performanceSpanId: 'span-api-2', level: 'INFERRED', evidence: '推测关联：标识不同；同一 Turn 内事件类型相符，时间重叠 92%。', timeDeltaMs: 200 },
