...
export type Completeness = '完整' | '缺少内容' | '缺少性能' | '部分缺失'
...
export interface TimelineItem {
  id: string
  layer: 'Hook 行为' | 'OTel 性能'
  lane: string
  title: string
  startMs: number
  durationMs: number
  status: 'success' | 'warning' | 'danger' | 'info'
  source: 'Hook' | 'Hook + JSONL' | 'OTel Trace'
  description: string
  input?: string
  output?: string
  raw: Record<string, unknown>
  hookRaw?: Record<string, unknown>
  jsonlRaw?: Record<string, unknown>
  contentStatus?: '已补齐' | '等待补齐' | '不适用'
  timingPrecision?: 'OTel 精确' | 'Hook 边界估算' | 'JSONL 时间戳估算'
  toolUseId?: string
  callId?: string
  traceId?: string
}
