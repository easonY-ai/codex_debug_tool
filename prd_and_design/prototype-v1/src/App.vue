...
const associatedOperation = computed(() => nodes.value.find(node => node.id === selected.value?.operationId))
const relatedSpans = computed(() => nodes.value.filter(node => node.operationId === selected.value?.id))
...
<template>
  <div class="app">
    <header class="topbar">
      ...
      <span class="prototype-label">V1.0 · MOCK</span>
    </header>
    ...
    <main v-else-if="page === 'trace'" class="page trace-page">
      ...
      <div class="trace-grid"><div class="trace-primary">
        <section class="section timeline-section"><div class="section-head"><div><h2>共享执行时间轴</h2><p>OTel 执行 · Transcript 内容 · OTel 性能</p></div><div class="zoom-controls"><button title="缩小" aria-label="缩小" @click="zoom = Math.max(1, zoom - .25)"><el-icon><ZoomOut /></el-icon></button><span>{{ Math.round(zoom * 100) }}%</span><button title="放大" aria-label="放大" @click="zoom = Math.min(2.5, zoom + .25)"><el-icon><ZoomIn /></el-icon></button></div></div>
          <TimelineChart :nodes="nodes" :duration="duration" :zoom="zoom" @inspect="inspect" />
        ...
        <el-tabs v-model="inspectorTab">
          <el-tab-pane label="证据" name="evidence">
            ...
            <div class="inspector-section">
              ...
              <p v-if="associatedOperation">关联操作：{{ associatedOperation.title }}</p>
              <p v-if="relatedSpans.length">关联 Span：{{ relatedSpans.map(span => span.title).join('、') }}</p>
              <p v-if="selected.source === 'OTel 性能' && !associatedOperation">操作归属：未知操作</p>
            </div>
          </el-tab-pane>
