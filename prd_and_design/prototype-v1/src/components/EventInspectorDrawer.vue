    ...
    <template v-if="item">
      <el-tabs v-model="activeTab" stretch>
        <el-tab-pane v-if="jsonl" label="输入 / 输出" name="输入 / 输出">
          <div class="drawer-source-note"><b>JSONL 行为事件</b><span>时间轴 +{{ seconds(jsonl.startMs) }}</span></div>
        ...
        <el-tab-pane label="原始数据" name="原始数据">
          <div v-if="jsonl" class="raw-source-card jsonl-source"><header><span>JSONL 原文</span><small>{{ jsonl.title }}</small></header><pre class="raw-view">{{ JSON.stringify(jsonl.raw, null, 2) }}</pre></div>
