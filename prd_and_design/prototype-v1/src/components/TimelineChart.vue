...
function renderItem(params: any, api: any) {
  ...
  const children: any[] = [{
    ...
    style: { fill: colors[item.status], opacity: item.layer === 'Agent 行为' ? 0.92 : 0.72, stroke: selected ? '#16213a' : '#ffffff', lineWidth: selected ? 3 : 1 },
  }]
...
<template>
  <div ref="root" class="timeline-chart" role="img" aria-label="Agent 行为和 OTel 性能双层共享时间轴，可滚轮缩放和拖动平移" />
</template>
